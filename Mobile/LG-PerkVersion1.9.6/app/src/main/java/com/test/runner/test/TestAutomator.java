...
public class TestAutomator extends UiAutomatorTestCase {
    ...
    public void testAutomate() throws UiObjectNotFoundException {
        ...
        // Create file to indicate perk automation has started.
        File startFile = new File(Environment.getExternalStorageDirectory() + File.separator + "started.txt");
        try { startFile.createNewFile(); } catch (IOException e) {
            Log.e("Start File Creation", e.getMessage());
        }
        // Delete file that indicates perk automation has stopped.
        File stopFile = new File(Environment.getExternalStorageDirectory() + File.separator + "stop.txt");
        stopFile.delete();

        while (!stopFile.exists()) {
            try {
                int hour = Calendar.getInstance().get(Calendar.HOUR_OF_DAY);
                if (hour < 5 || hour > 23) { // Rest
                ...
                if (obj.exists()) {
                    ...
                    Process process = Runtime.getRuntime().exec("getprop pPass");
        ...
        // Remove files to reset perk automation state.
        startFile.delete();
        stopFile.delete();
    }
}